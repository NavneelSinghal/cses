...
vector<int> get_match_positions(string& to_find, const string& text) {
    ...
    for (int i = 1, j = 0; i < total_size; ++i) {
        ...
        if (i <= n)
            ...
            match_positions.push_back(i - 2 * n);
    }
...
int main() {
    ...
    for (int _test = 1; _test <= _tests; ++_test) {
        ...
        int k;
        cin >> k;
        vector<vector<int>> at_position(n);
        for (int i = 0; i < k; ++i) {
            string t;
            cin >> t;
            sizes.push_back((int)t.size());
            auto pos = get_match_positions(t, s);
            for (auto p : pos) at_position[p].push_back(i);
        }
        vector<mint> dp(n + 1);
        dp[n] = 1;
        for (int i = n - 1; i >= 0; --i) {
            mint x = 0;
            for (auto matching_string_index : at_position[i])
                x += dp[i + sizes[matching_string_index]];
            dp[i] = x;
        }
        cout << dp[0] << '\n';
    }
