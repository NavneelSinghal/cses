...
constexpr int N = 1e6;
vector<mint> fact(N + 1), ifact(N + 1), inv(N + 1);
...
int main() {
    ...
    for (int _test = 1; _test <= _tests; ++_test) {
        ...
        if (!works || stack_size > n - (int)s.size() || (stack_size & 1) != ((n - (int)s.size()) & 1))
            cout << 0 << '\n';
        else {
            // ( -> right
            // ) -> up
            // we want to go from (r, u) to (n/2, n/2)
            // without crossing y = x
            n /= 2;
            cout << C(2 * n - r - u, n - r) - C(2 * n - r - u, n - r - 1)
                 << '\n';
        }
    }
}
