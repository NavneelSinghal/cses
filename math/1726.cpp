...
void solve() {
    ...
    double dp[n + 1][8][8];
    double ans[8][8];
    rep(i, 8) rep(j, 8) ans[i][j] = 1;
    double fin = 0;
    rep(i, 8) rep(j, 8) {
        rep(k, n + 1) rep(ii, 8) rep(jj, 8) dp[k][ii][jj] = 0;
        dp[0][i][j] = 1;
        rep(k, n) rep(ii, 8) rep(jj, 8) {
            int dirs = (ii > 0) + (ii < 7) + (jj > 0) + (jj < 7);
            if (ii > 0) dp[k + 1][ii - 1][jj] += dp[k][ii][jj] / dirs;
            if (ii < 7) dp[k + 1][ii + 1][jj] += dp[k][ii][jj] / dirs;
            if (jj > 0) dp[k + 1][ii][jj - 1] += dp[k][ii][jj] / dirs;
            if (jj < 7) dp[k + 1][ii][jj + 1] += dp[k][ii][jj] / dirs;
        }
        rep(ii, 8) rep(jj, 8) ans[ii][jj] *= (1 - dp[n][ii][jj]);
    }
    rep(i, 8) rep(j, 8) fin += ans[i][j];
    cout << fin << endl;
}
