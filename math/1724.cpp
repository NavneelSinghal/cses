        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < a.cols; j++) {
                ...
                for (int k = 0; k < cols; k++) {
                    ...
                    if (mat[i][k] != mod * mod && a.mat[k][j] != mod * mod)
                        res.mat[i][j] =
                            min(res.mat[i][j], mat[i][k] + a.mat[k][j]);
                }
            }
        }
        return res;
...
void solve() {
    ...
    Matrix<int> m(n, n);
    m.fill(mod * mod);
    auto id = Matrix<int>::id(n);
    id.fill(mod * mod);
    rep(i, n) id.mat[i][i] = 0;
    while (m_--) {
        int u, v;
        cin >> u >> v;
        --u, --v;
        int w;
        cin >> w;
        m.mat[u][v] = min(m.mat[u][v], w);
    }
    Matrix<int> res = power(m, id, k);
