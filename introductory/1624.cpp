...
void solve() {
    ...
    rep(i, 8) cin >> board[i];
    int sols = 0;
    function<void(int, int, vi&, vi&, vi&)> search =
        [&](int previ, int numqueens, vi& cols, vi& sums, vi& diff) {
            if (numqueens == 8) {
                sols++;
                return;
            }
            for (int i = previ + 1; i < 8; i++) {
                for (int j = 0; j < 8; j++) {
                    if (board[i][j] == '.' &&
                        find(all(cols), j) == cols.end() &&
                        find(all(sums), i + j) == sums.end() &&
                        find(all(diff), i - j) == diff.end()) {
                        cols.pb(j);
                        sums.pb(i + j);
                        diff.pb(i - j);
                        search(i, numqueens + 1, cols, sums, diff);
                        cols.pop_back();
                        sums.pop_back();
                        diff.pop_back();
                    }
    ...
    vector<int> cols, sums, diff;
    search(-1, 0, cols, sums, diff);
    cout << sols << endl;
}
