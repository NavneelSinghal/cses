...
int main() {
    ...
    for (int _test = 1; _test <= _tests; ++_test) {
        // cout << "Case #" << _test << ": ";
        /*
           first suppose that the graph G is connected
           consider any eulerian subgraph of G
           let there be a cycle u_0 -> u_1 -> ... -> u_k -> u_{k + 1} -> u_0,
           where k >= 1.

           consider the edge (u_0, u_1).
           we claim that there are exactly as many eulerian subgraphs of G with
           e as without e. for a proof, take any eulerian subgraph of G with e.
           now flip all incidences of the edges in the cycle in the eulerian
           subgraph. i.e., if S is the set of edges of this cycle which are in
           the subgraph, and S' = set of the remaining vertices, we swap S and
           S' to get another Eulerian subgraph.

           so when we remove an edge that is in a cycle, we halve the number of
           eulerian subgraphs. note that we can find a cycle as long as m > n -
           1, and breaking a cycle this way reduces m by exactly 1, so the
           answer is 2^(m - n + 1) for a connected graph.

           taking the product over components, it is 2^(m - n +
        ...
        int components = 0;
        for (int i = 0; i < n; ++i)
            if (!visited[i]) ++components, dfs(dfs, i);
        cout << mint(2).pow(m - n + components) << '\n';
    }
}
