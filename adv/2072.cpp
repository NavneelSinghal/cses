...
namespace treap {
    ...
    void update_cnt(treap_node_ptr t) {
        if (t) t->cnt = cnt(t->left_child) + cnt(t->right_child) + 1;
    }
...
int main() {
    ...
    for (int _test = 1; _test <= _tests; ++_test) {
        ...
        while (q--) {
            int u, v;
            cin >> u >> v;
            treap::treap_node_ptr a, b, c, d;
            treap::split(root, a, b, u - 1);
            treap::split(b, c, d, v - u + 1);
            treap::merge(b, a, d);
            treap::merge(root, b, c);
        }
        vector<int> a;
        treap::inorder_traversal(root, a);
