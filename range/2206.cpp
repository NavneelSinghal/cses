...
int main() {
    ...
    for (int _test = 1; _test <= _tests; ++_test) {
        ...
        using Base = int;
        using Node = int;
        constexpr Node id_node = 2e9;
        auto make_node = [](int x, int) {
            return x;
        };
        ...
        constexpr Update id_update = nullptr;
        auto apply_update = [](Update u, const Node& n) -> Node {
            return n;
        };
        lazy_segtree st_plus(P, id_node, make_node, combine, id_update, apply_update);
        lazy_segtree st_minus(Q, id_node, make_node, combine, id_update, apply_update);
        while (q--) {
            int t;
            cin >> t;
            if (t == 1) {
                ...
                cin >> k;
                --k;
                int left_min = st_minus.query(0, k + 1) + k;
                int right_min = st_plus.query(k, n) - k;
