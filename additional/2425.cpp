...
int main() {
    ...
    for (int _test = 1; _test <= _tests; ++_test) {
        ...
        lazy_segtree st(v, id_node, make_node, combine, id_update, apply_update,
                        compose_updates);
        // st[i, i + 1) = how much the first person is ahead of the second
        // person when starting from the last element
        for (int i = 0; i < n; ++i) {
            int x, t;
            cin >> x >> t;
            st.update(0, x, 3 - 2 * t);
            auto [mn, mx] = st.all_query();
            if (mx <= 0)
                cout << "<\n";
            else if (mn >= 0)
                cout << ">\n";
