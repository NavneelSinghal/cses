...
int cost(vi& a, int num) {
    int ans = 0;
    rep(i, a.size()) ans += abs(a[i] - num);
    return ans;
}
...
void solve() {
    ...
    int idx = n / 2;
    int idx2 = n / 2 - 1;
    if (idx2 >= 0) ans = min(ans, cost(a, a[idx2]));
    ans = min(ans, cost(a, a[idx]));
    if (idx >= 0) ans = min(ans, cost(a, (a[idx] + a[idx2]) / 2));
    if (idx >= 0) ans = min(ans, cost(a, 1 + (a[idx] + a[idx2]) / 2));
    cout << ans << endl;
}
