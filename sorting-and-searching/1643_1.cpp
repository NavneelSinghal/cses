...
node combine(node n1, node n2) {
    ...
    ans.left = max({0ll, n1.left, n1.sum + n2.left});
    ans.right = max({0ll, n2.right, n2.sum + n1.right});
    ans.mx = max({0ll, n1.mx, n2.mx, n1.right + n2.left});
    return ans;
}
...
void solve() {
    ...
    rep(i, n) cin >> a[i];
    build(1, 0, n - 1, a);
    int ans = query(1, 0, n - 1, 0, n - 1).mx;
    if (ans == 0) {
        ans = a[0];
