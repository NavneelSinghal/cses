...
int main() {
    ...
    for (int _test = 1; _test <= _tests; ++_test) {
        ...
        int n, m;
        cin >> n >> m;
        vector dp(m + 1, vector(1 << n, mint(0)));
        auto solve = [&](const auto& self, int col, int row, int cur_mask,
                         int next_mask) -> void {
            if (col == m) return;
            if (row >= n) {
                dp[col + 1][next_mask] += dp[col][cur_mask];
                return;
            }
            int mask_here = 1 << row;
            if (cur_mask & mask_here) {
                // there is an element at the required position
                // just go ahead with the next row
                self(self, col, row + 1, cur_mask, next_mask);
            } else {
                // put a row domino here:
                self(self, col, row + 1, cur_mask, next_mask | mask_here);
                // see if it is possible to put a col domino here
                if (row + 1 < n && !(cur_mask & (mask_here << 1)))
                    self(self, col, row + 2, cur_mask, next_mask);
            }
        };
        // dp[i][j] = first i columns filled, and the mask of the last row is j
        dp[0][0] = 1;
        for (int col = 0; col < m; ++col)
            for (int mask = 0; mask < (1 << n); ++mask)
                solve(solve, col, 0, mask, 0);
        cout << dp[m][0] << '\n';
    }
}
