...
int main() {
    ...
    vector<mint> dp_split(N), dp_joined(N);
    dp_split[1] = 1;
    dp_joined[1] = 1;
    constexpr mint four = 4, two = 2;
    for (int i = 2; i < N; ++i) {
        dp_split[i] = four * dp_split[i - 1] + dp_joined[i - 1];
        dp_joined[i] = dp_split[i - 1] + two * dp_joined[i - 1];
    }
